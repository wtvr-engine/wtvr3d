...
use nalgebra::{Translation3,UnitQuaternion,Vector3,Matrix4,Isometry3};
use specs::{Component,VecStorage,FlaggedStorage,DenseVecStorage,Entity};
...
impl Transform {
    ...
    pub fn new(translation : &Vector3<f32>, rotation : &Vector3<f32>, scale : &Vector3<f32>) -> Transform {
        Transform {
    ...
    pub fn set_rotation(&mut self, new_rotation : &Vector3<f32>) -> () {
        self.local_rotation = UnitQuaternion::from_euler_angles(new_rotation.x,new_rotation.y,new_rotation.z);
        self.dirty = true;
    }
    ...
    pub fn refresh_world_matrix(&mut self, parent_world_matrix : Matrix4<f32>) -> () {
        ...
        let isometry = Isometry3::from_parts(self.local_translation.clone(), self.local_rotation.clone());
    ...
    pub fn get_world_matrix(&self) -> Result<Matrix4<f32>,String> {
        if self.dirty {
            Err(String::from("Trying to get world matrix while it is dirty!"))
        } 
        else {
