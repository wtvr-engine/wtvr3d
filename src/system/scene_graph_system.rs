...
impl<'a> System<'a> for SceneGraphSystem {
    type SystemData = (
        ...
        ReadStorage<'a, DirtyTransform>,
        ReadStorage<'a, Enabled>,
    );
    fn run(&mut self, (entities, hierarchy, mut transforms, dirty, enabled): Self::SystemData) {
        let mut dirty_transforms = HashMap::new();
        for (entity, _, _, _) in (&entities, &mut transforms, &dirty, &enabled).join() {
        ...
        for (entity,parent_entity_opt) in dirty_transforms {
            let parent_matrix = if let Some(parent) = parent_entity_opt {
                if let Ok(matrix) = transforms.get(parent).unwrap().get_world_matrix() {
                    Some(matrix)
                }
                else{
                    None
